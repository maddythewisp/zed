...
use crate::{
    ...
    PrimitiveBatch, Quad, ScaledPixels, Scene, SceneSegmentPool, Shadow, Size, Underline,
...
impl BladeRenderer {
    ...
    pub fn draw(&mut self, scene: &Scene, segment_pool: &SceneSegmentPool) {
        ...
        let context_transforms =
            unsafe { self.instance_belt.alloc_typed(&gpu_transforms, &self.gpu) };
                ...
                PrimitiveBatch::Paths(paths) => {
                    ...
                    let sprites = if paths.last().unwrap().order == first_path.order {
                        paths
                            ...
                            .filter_map(|path| {
                                let t =
                                    resolve_world_transform(path.transform_index, &gpu_transforms);
                                let world_bounds = transform_bounds(path.clipped_bounds(), t)
                        ...
                        bounds
                            .map(|bounds| vec![PathSprite { bounds }])
                            .unwrap_or_default()
                    };
                    if sprites.is_empty() {
                        continue;
